...
All file paths are defined relative to the project root. The raw inputs this script reads
(BillSum, EUR-Lex, and the four raw law texts) are publicly available from their original
sources; the processed datasets are on Zenodo (DOI 10.5281/zenodo.20749943). See
`data/README.md` for the full file-by-file breakdown, then place everything inside `data/`
...
3. Data quality diagnostics (word counts)
...
# Random Sample Validation (LaTeX Export)
...
```
# External Validation: Law Parsing

```{r}
# Paths to raw law text files — place these in data/raw/
aca_path   <- "data/raw/aca.txt"
tcja_path  <- "data/raw/tcja.txt"
gdpr_path  <- "data/raw/gdpr.txt"
bbb_path   <- "data/raw/bbb.txt"


aca_text  <- read_law_text(aca_path)
tcja_text <- read_law_text(tcja_path)
gdpr_text <- read_law_text(gdpr_path)
bbb_text  <- read_law_text(bbb_path)
...
parse_us_law <- function(text, law_title) {
  ...
  tibble(
    ...
    section_full = str_squish(sections[-1])
  )
}
...
parse_gdpr <- function(text, law_title) {
  ...
  tibble(
    ...
    section_full = str_squish(articles[-1])
  )
}
aca_df  <- parse_us_law(aca_text,  "Affordable Care Act (2010)")
tcja_df <- parse_us_law(tcja_text, "Tax Cuts and Jobs Act (2017)")
bbb_df  <- parse_us_law(bbb_text,  "One Big Beautiful Bill Act")
gdpr_df <- parse_gdpr(gdpr_text,   "EU General Data Protection Regulation (2016)")

# Combine all laws
laws_df <- bind_rows(aca_df, tcja_df, bbb_df, gdpr_df) %>%
  filter(section_full != "")
...
```{r}
# 3. Data Quality: Word Count Diagnostics
...
# Remove all observations where section is less than 10 words

```{r}
df_all <- df_all %>%
  mutate(
    section_word_count = str_count(section, "\\S+")
  ) %>%
  filter(
    !is.na(section),
    section_word_count >= 10
  ) %>%
  select(-section_word_count)
```

# Remove text and section variables to reduce file size
