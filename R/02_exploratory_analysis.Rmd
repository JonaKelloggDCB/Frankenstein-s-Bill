---
title: "LegalTopicAnalysis — Exploratory"
...
Welcome to the exploratory analysis for the Frankenstein's Bill project. This script produces descriptive plots and validity regressions for `divergence_vs_title` and `hiddenness_textual`.

# Load Data

`df_top10.csv` is provided in the Zenodo archive (DOI 10.5281/zenodo.20749943):
https://doi.org/10.5281/zenodo.20749943

Download `df_top10.csv` (unzip `df_top10.csv.zip`) and place it in the `data/` folder before
running. See `data/README.md` for the full data breakdown.

```{r}
df_all <- read.csv("data/df_top10.csv")
...
ggsave(
  filename = "output/combocount.pdf",
  plot = p_hiddenness,
  width = 6.5,
  height = 5,
  units = "in"
...
```{r}
ggsave(
  "output/box_hiddenness_title.pdf",
...
ggsave(
  "output/box_hiddenness_section.pdf",
...
ggsave(
  "output/box_divergence_title.pdf",
...
ggsave(
  "output/box_divergence_section.pdf",
...
```{r}
ggsave(
  "output/box_hiddenness_country.pdf",
...
ggsave(
  "output/box_divergence_country.pdf",
...
`combo_counts` must be created upstream by counting section–title label combinations,
e.g.: combo_counts <- df_all %>% count(title_label, section_label) %>% filter(title_label != "none", section_label != "none")
...
combo_counts <- df_all %>%
  count(title_label, section_label) %>%
  filter(title_label != "none", section_label != "none")
...
plot2 <- ggplot(combo_counts, aes(
  ...
  labs(
    x = "Title Subject",
    y = "Section Subject",
    title = "Section-Title Subject Combinations (Excluding None)"
  ) +
  theme_minimal(base_size = 12) +
...
```{r}
ggsave(
  "output/hiddenness_over_time_by_country.pdf",
...
ggsave(
  "output/divergence_over_time_by_country.pdf",
...
## Topic and Textual Divergence

Note: m_diff_section, m_diff_title, m_diff_both must be defined by running analogous
regressions with divergence_vs_title as the outcome (parallel to the hiddenness models above).
`textual_difference` is an older variable name; use `divergence_vs_title` throughout.

```{r}
# Section only
m_diff_section <- lm(
  divergence_vs_title ~ section_label,
  data = df_all
)

# Title only
m_diff_title <- lm(
  divergence_vs_title ~ title_label,
  data = df_all
)

# Section + Title
m_diff_both <- lm(
  divergence_vs_title ~ section_label + title_label,
  data = df_all
)
...
stargazer(
  ...
  m_diff_both,
  type = "latex",
  title = "Textual Divergence by Policy Subject",
...
Note: `divergence_vs_title` is used here (TF-IDF cosine divergence, section vs bill title).
An older variable `textual_difference` was removed; the two referred to the same concept.
...
stargazer(
  ...
  type = "latex",
  title = "Textual Hiddenness and Divergence by Country",
  dep.var.labels = c(
    "Textual Hiddenness",
    "Textual Divergence from Title"
  ),
  column.labels = c(
    "Hiddenness",
    "Divergence"
